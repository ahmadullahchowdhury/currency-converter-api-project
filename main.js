...
const UI = {
    ...
    to:'', 
    loadSelector() {
        const fromElm = document.querySelector('#fromForm')
        const toElm = document.querySelector('#toForm')
    ...
    clearInput(){
        const {fromElm, toElm, amountElm } = this.loadSelector()
        fromElm.value  = 'From'
        toElm.value  = 'To'
        amountElm.value = ''
},
...
hideMessage() {
  ...
  showMessage(msg) {
    ...
    msgElm.innerHTML = elm
    this.hideMessage()
  },
  validateInput(from, to, currencyValue)
  {if (from === '' || from === 'From' || to === '' ||  to === 'To'|| parseInt(currencyValue) === '' || parseInt(currencyValue) <= 0 ) {
  ...
  getInput() {
    const { fromElm, toElm, amountElm } = this.loadSelector()
    const from = fromElm.value
    const to = toElm.value
    const currencyValue = amountElm.value
    
    const isValidated = this.validateInput(from, to, currencyValue)
 
    return { from, to, currencyValue, isValidated  }
  },
  async getAndPopulateUI() {
    console.log(this)
   
    const { from, to, currencyValue} = storage.getLocalData()
    
    currencyData.from = from
    currencyData.to = to
    currencyData.currencyValue = currencyValue

    this.from = from
    this.to = to
    this.currencyValue = currencyValue

    const data = await currencyData.getApiData()
  ...
  populateUI(data) {
    ...
    const result = data[`${this.from}_${this.to}`]*this.currencyValue
    console.log(result)
    
 
    fromOutputElm.textContent = `${this.currencyValue} ${currencyNameList[this.from].currencyName}`
    toOutputElm.textContent = `${result} ${currencyNameList[this.to].currencyName}`

  },
  init(){
    const {fromElm, toElm, buttonElm, amountElm} = this.loadSelector()

    for(i in currencyNameList){
        const dropDownList1 = document.createElement("option")
        //dropDownList1.textContent = currencyNameList[i].currencyName+ "" + "-" + "["+currencyNameList[i].id+"]"
        dropDownList1.textContent = currencyNameList[i].id 
        //dropDownList1.textContent = `${currencyNameList[i].currencyName}`

        const dropDownList2 = document.createElement("option")
        dropDownList2.textContent = currencyNameList[i].id
        //dropDownList2.textContent = currencyNameList[i].currencyName+ "" + "-" + "["+currencyNameList[i].id+"]"

        fromElm.insertAdjacentElement('beforeend',dropDownList1)
        toElm.insertAdjacentElement('beforeend',dropDownList2)
    }

    buttonElm.addEventListener('click', async e =>{
        //e.preventDefault()
        const { from, to, currencyValue, isValidated  }  = this.getInput()
        if(isValidated){
            ...
            storage.from = from
            storage.to = to
            storage.currencyValue = currencyValue
...
const currencyData = {
    ...
    async getApiData(){
        try{
            const res =   await fetch(`https://free.currconv.com/api/v7/convert?q=${this.from}_${this.to}&compact=ultra&apiKey=b9b59361ba623dd1b0f7`)
...
const storage = {
    ...
    to: '',
    saveData(){
        localStorage.setItem('from_currency',this.from)
        localStorage.setItem('to_currency',this.to)
